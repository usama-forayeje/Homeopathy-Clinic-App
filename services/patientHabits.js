...
const PATIENT_HABITS_COLLECTION_ID = process.env.NEXT_PUBLIC_COLLECTION_PATIENT_HABITS_ID

const patientHabitsService = {
  async getPatientHabitsByPatientId(patientId) {
    try {
      const response = await databases.listDocuments(DATABASE_ID, PATIENT_HABITS_COLLECTION_ID, [
  ...
  async createPatientHabit(habitData) {
    try {
      const response = await databases.createDocument(DATABASE_ID, PATIENT_HABITS_COLLECTION_ID, ID.unique(), habitData)
      return response
    } catch (error) {
  ...
  async updatePatientHabit(habitId, habitData) {
    try {
      const response = await databases.updateDocument(DATABASE_ID, PATIENT_HABITS_COLLECTION_ID, habitId, habitData)
      return response
    } catch (error) {
  ...
  async deletePatientHabit(habitId) {
    try {
      await databases.deleteDocument(DATABASE_ID, PATIENT_HABITS_COLLECTION_ID, habitId)
      return true
    } catch (error) {
  ...
  async updatePatientHabits(patientId, updatedHabitsArray) {
    ...
    const updatedHabitIds = new Set(updatedHabitsArray.map((h) => h.$id).filter((id) => id))

    const promises = []

    // Delete habits that are no longer in updatedHabitsArray
